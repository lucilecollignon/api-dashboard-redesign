...
La première étape dans la réalisation du tableau de bord consiste à définir les données qui seront utilisées.
La bibliothèque supporte de manière standard différents [fournisseurs](https://github.com/geo2france/api-dashboard/tree/main/src/data_providers).
...
traduits dans le format attendu par le fournisseur de données. Voir la [liste des opérateurs](https://github.com/geo2france/api-dashboard/blob/b9c1ddb511f6bf722704b4935160eacdcfe33cc2/src/data_providers/types.ts#L23-L45). Attention, les fournisseurs ne supportent pas toujours tous les opérateurs.

```jsx
<Filter field='L_REGION'>Hauts-de-France</Filter> // opérateur eq (égalité) par défaut
...
Le WFS permet d'alléger les requêtes en ne demandant que les champs nécessaires (https://docs.geoserver.org/main/en/user/services/wfs/reference.html#getfeature).
...
Idéalement, on utilisera la même propriété meta pour les datasets liés au même fournisseur, afin de réduire le nombre de requêtes sur le serveur.
